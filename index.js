...
// Prevent process crashes on Discord API timeouts
...
// =========================
// GROQ AI (Used for Battles, Adventure, Summaries)
// =========================
const ai = new OpenAI({
...
const activeBatBattles = new Set();
...
client.on(Events.InteractionCreate, async interaction => {
  try {
    ...
    // --- /ASK (POWERED EXCLUSIVELY BY GEMINI INTERACTIONS API) ---
...
3. Include Doraemon gadgets such as Anywhere Door, Bamboo Copter, Time Machine, Small Light, Big Light, Translation Jelly, Momotaro Dumpling (to control animals) etc.
4. Return ONLY a raw JSON object with no markdown formatting.
5. Journeys should be set in Japan and Globally or Outer Space or Future/Past rarely, should not be about nobita's ancestors or something too much.
...
Structure: {"story":"...","choices":["Choice A","Choice B","Choice C"]}`;
      ...
      try {
        ...
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId('adv_0').setLabel(gameData.choices[0]).setStyle(ButtonStyle.Primary),
          new ButtonBuilder().setCustomId('adv_1').setLabel(gameData.choices[1]).setStyle(ButtonStyle.Primary),
          new ButtonBuilder().setCustomId('adv_2').setLabel(gameData.choices[2]).setStyle(ButtonStyle.Primary)
        );
        ...
        collector.on('collect', async (i) => {
          ...
          let userPrompt = `I choose: ${userChoice}. Continue the Doraemon adventure and return the next story beat with 3 new choices in the same JSON format.`;

          if (session.turn >= 5) {
            userPrompt = `I choose: ${userChoice}. This is the final action. End the Doraemon adventure with a fun, emotional, or surprising ending. Do NOT generate new choices. Return this exact JSON format: {"story":"Your ending here...","choices":[]}`;
          }
          ...
          if (nextJson.startsWith('```json')) {
            nextJson = nextJson.replace(/```json\n?/, '').replace(/```/, '');
          }
    ...
    if (interaction.commandName === 'battle') {
      ...
      try {
        ...
        while (round <= 2 && !winner) {
          ...
          p1Move = movesData.c1_moves[p1Index].name;
          ...
          const evalPrompt = round === 1 
            ...
            : `Evaluate this final clash: ${character1} uses ${p1Move} vs ${character2} uses ${p2Move}. Declare a definitive winner between ${character1} and ${character2}. You MUST choose one winner. Format: {"is_stall": false, "narrative": "A fast-paced 2-3 sentence final showdown.", "winner": "Name"}`;
        ...
        if (winLower.includes(c2Lower) || c2Lower.includes(winLower)) {
...
client.on('messageCreate', async (message) => {
  ...
  if (
    ...
    (message.content.toLowerCase() === '!sum' || message.content.toLowerCase() === '!summary')
  ) {
    try {
      ...
      const response = await ai.chat.completions.create({
        model: 'llama-3.3-70b-versatile',
        messages: [
          { role: 'system', content: 'Give a short title and summarize the text in 3-5 bullet points.' },
  ...
  if (message.content.toLowerCase() === '!superover') {
    ...
    return message.reply({
      embeds: [{
        ...
        color: 0x00FFFF,
        fields: [
          { name: 'First 5 Balls', value: balls.join(' • '), inline: false },
          { name: 'Score', value: `**${score}/${wickets}**`, inline: true },
          { name: 'Need', value: `**${needed} off 1 ball**`, inline: true },
          {
            name: 'Type your shot',
            value: '`defensive` 🛡️\n`drive` 🏏\n`loft` 🚀\n`scoop` 🪄',
            inline: false
        ...
        footer: { text: 'Reply with defensive, drive, loft, or scoop within 15 seconds' }
      }]
    });
  }
  ...
  if (['defensive', 'drive', 'loft', 'scoop'].includes(message.content.toLowerCase())) {
    ...
    let finalBall;
    if (shot === 'defensive') finalBall = [0, 1, 1, 2, 2, 'W'][Math.floor(Math.random() * 6)];
    else if (shot === 'drive') finalBall = [0, 1, 2, 3, 4, 'W'][Math.floor(Math.random() * 6)];
    else if (shot === 'loft') finalBall = [0, 0, 2, 4, 6, 'W'][Math.floor(Math.random() * 6)];
    else finalBall = [0, 0, 4, 6, 6, 'W'][Math.floor(Math.random() * 6)];
    ...
    return message.reply({
      embeds: [{
        ...
        fields: [
          {
            name: 'Shot',
            value: shot === 'defensive' ? '🛡️ Defensive' : shot === 'drive' ? '🏏 Straight Drive' : shot === 'loft' ? '🚀 Lofted Shot' : '🪄 Scoop',
            inline: true
          },
          { name: 'Ball 6', value: finalBall === 'W' ? '💥 WICKET' : `**${finalBall}**`, inline: true },
          { name: 'Final Score', value: `**${finalScore}/${finalWickets}**`, inline: true },
          {
            name: 'Result',
            value: won ? '🔥 **YOU WIN THE SUPER OVER!**' : '😬 **YOU LOSE THE SUPER OVER!**',
            inline: false
          }
        ]
      }]
  ...
  if (
    ...
    !message.content.toLowerCase().startsWith('!wanted')
  ) return;
    ...
    return message.reply({
      embeds: [{
        ...
        fields: [
          ...
          { name: '🧠 Memory', value: '**2h active**', inline: true }
        ],
        footer: { text: 'Mention me with a question to start chatting' }
      }]
    });
  }
